...
const cartController = {
    ...
    getCart: async (req, res) => {
        try {
            ...
            if (order) {
                ...
                return res.status(200).json({ data: cartWithBooks, order_id: order.id });
            } else {
    ...
    deleteCart: async (req, res) => {
        try {
            ...
            const user_id = req.userId;
            const order = await db.Order.findOne({ where: { user_id, status: "-1" } });
            const bookItem = await db.OrderDetail.findOne({ where: { order_id: order.id, book_id: book_id } });
